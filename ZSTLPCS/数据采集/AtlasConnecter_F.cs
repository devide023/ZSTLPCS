...
namespace ZSTJPCS
{
    ...
    /// 
    /// </summary>
    public class AtlasConnecter_F : DataCollecter
    {
        #region 构造函数

        AtlasConnecter_F(AtlasControlerType as_AtlasControlerType = AtlasControlerType.PF4000)
        {
            ...
            for (int i = 0; i < 100; i++)//序号从0开始没错，因为第一个是不用的
            {
                ...
                Cmd_ChangePSetIndex.Add(new AtlsCommand( CommcationType,18, 0, i.ToString().PadLeft(3, '0')));
                Cmd_LoadPSetIndex.Add(new AtlsCommand(CommcationType, 12, 1, i.ToString().PadLeft(3, '0')));
            }
            for (int i = 0; i < 100; i++)
            {
                //byte[] b = PackCommand_Socket(38, 0, i.ToString().PadLeft(2, '0'));
                //Cmd_ChangeJSetIndex.Add(new AtlsCommand(b));
                Cmd_ChangeJSetIndex.Add(new AtlsCommand(CommcationType, 38, 1, i.ToString().PadLeft(2, '0')));
                Cmd_LoadJSetIndex.Add(new AtlsCommand(CommcationType, 32, 0, i.ToString().PadLeft(2, '0')));
            }
            for (int i = 0; i < 50; i++)
            {
                Cmd_ChangeMSetIndex.Add(new AtlsCommand(CommcationType, 2606, 0, i.ToString().PadLeft(4, '0')));
            }

            #endregion
            Cmd_Connect = new AtlsCommand(CommcationType, 1, 0, "");
            Cmd_DisConnect = new AtlsCommand(CommcationType, 3, 0, "");
            Cmd_Refuse = new AtlsCommand(CommcationType, 4, 0, "");
            //public static AtlsCommand CurveSub = PackCommand_SerialPort(8, 0, "0900001350                             01001");
            Cmd_CurveSub = new AtlsCommand(CommcationType, 8, 0, "0900001350                             02001002");
            Cmd_TracePlottingParameter = new AtlsCommand(CommcationType, 8, 0, "090100100");
            Cmd_AskPSetIndex = new AtlsCommand(CommcationType, 10, 0, ""); 
            Cmd_AskJSetIndex = new AtlsCommand(CommcationType, 30, 0, "");
            Cmd_Lock = new AtlsCommand(CommcationType, 42, 0, "");
            Cmd_UnLock = new AtlsCommand(CommcationType, 43, 0, "");
            Cmd_ResultSub = new AtlsCommand(CommcationType, 60, 0, "");
            Cmd_ResultACK = new AtlsCommand(CommcationType, 62, 0, "");
            Cmd_MultiResultSub = new AtlsCommand(CommcationType, 100, 0, "");
            Cmd_MultiResultACK = new AtlsCommand(CommcationType, 102, 0, "");
            Cmd_Heart = new AtlsCommand(CommcationType, 9999, 0, "");
            Cmd_JobInfoSubscribe = new AtlsCommand(CommcationType, 34, 1, "");
            Cmd_JobInfoAcknowledge = new AtlsCommand(CommcationType, 36, 1, "");
            Cmd_abortJob = new AtlsCommand(CommcationType, 127, 1, "");
        }

        /// <summary>以网络连接的方式初始化阿特拉斯拧紧轴连接驱动
        /// 
        /// </summary>
        /// <param name="as_ip">IP地址和端口号</param>
        /// <param name="as_AtlasControlerType"></param>
        public AtlasConnecter_F(IPEndPoint as_ip, AtlasControlerType as_AtlasControlerType = AtlasControlerType.PF4000)
            : this(as_AtlasControlerType)
        {
            CommcationType = CommcationType_Enum.TCP;
            ControlerIP = as_ip.Address.ToString();
            ControlerPort = as_ip.Port;
        }

        /// <summary>以串口连接的方式初始化阿特拉斯拧紧轴连接驱动
        /// 
        /// </summary>
        /// <param name="as_comparm">比如Com1,9600,N,8,1</param>
        /// <param name="as_AtlasControlerType"></param>
        public AtlasConnecter_F(string as_ComConfigStr, AtlasControlerType as_AtlasControlerType = AtlasControlerType.PF4000)
            : this(as_AtlasControlerType)
        {
            CommcationType = CommcationType_Enum.SerialPort;
            #region 串口参数注入
            var ss = as_ComConfigStr.Split(new char[] { ',', '，' }, StringSplitOptions.RemoveEmptyEntries);
            if (ss.Length >= 1)
            {
                sp.PortName = ss[0];
            }
            if (ss.Length >= 2)
            {
                if (int.TryParse(ss[1], out int int_temp))
                {
                    sp.BaudRate = int_temp;
                }
            }
            if (ss.Length >= 3)
            {
            ...
            if (ss.Length >= 4)
            {
                if (int.TryParse(ss[3], out int int_temp))
                {
                    sp.DataBits = int_temp;
                }
            }
            if (ss.Length >= 5)
            {
        ...
        /// 
        /// </summary>
        AtlsCommand Cmd_Connect;

        /// <summary>关闭连接,程序退出时使用
        /// 
        /// </summary>
        AtlsCommand Cmd_DisConnect;

        /// <summary>拒绝请求
        /// 
        /// </summary>
        AtlsCommand Cmd_Refuse;
        ...
        {
            return new AtlsCommand(CommcationType,5, 0, mid.ToString().PadLeft(4, '0'));
        }

        /// <summary>请求曲线数据_角度和扭力
        /// 
        /// </summary>
        AtlsCommand Cmd_CurveSub;

        /// <summary>请求曲线参数
        /// 
        /// </summary>
        AtlsCommand Cmd_TracePlottingParameter;
        ...
        {
            return new AtlsCommand(CommcationType,82, 0, dt.ToString("yyyy-MM-dd:HH:mm:ss"));
        }

        /// <summary>请求程序索引号PSet
        /// 
        /// </summary>
        AtlsCommand Cmd_AskPSetIndex;
        ...
        /// 
        /// </summary>
        AtlsCommand Cmd_Lock;

        /// <summary>解锁扭矩枪,允许枪动作
        /// 
        /// </summary>
        AtlsCommand Cmd_UnLock;

        /// <summary>订阅拧紧结果
        /// 
        /// </summary>
        AtlsCommand Cmd_ResultSub;

        /// <summary>拧紧结果确认
        /// 
        /// </summary>
        AtlsCommand Cmd_ResultACK;

        /// <summary>多轴结果订阅
        /// 
        /// </summary>
        AtlsCommand Cmd_MultiResultSub;

        /// <summary>多轴结果确认
        /// 
        /// </summary>
        AtlsCommand Cmd_MultiResultACK;

        /// <summary>心跳,至少每15秒发送一次,否则控制器会断开连接
        /// 
        /// </summary>
        AtlsCommand Cmd_Heart;

        /// <summary>订阅Job结果
        /// 
        /// </summary>
        AtlsCommand Cmd_JobInfoSubscribe;

        /// <summary>Job结果确认
        /// 
        /// </summary>
        AtlsCommand Cmd_JobInfoAcknowledge;

        /// <summary>关闭job任务
        /// 
        /// </summary>
        AtlsCommand Cmd_abortJob;
        ...
        {
            PF4000,
            /// <summary>目前并未发现PF6000和PF4000的驱动上有什么差异
            /// 
            /// </summary>
            PF6000,
            IRC_Focus,
        ...
        public enum CommcationType_Enum
        {
            SerialPort,
            TCP,
        }
        ...
        /// 
        /// </summary>
        public CommcationType_Enum CommcationType{get;set;}
        ...
        /// <returns></returns>
        bool CommcationSendData(byte[] b, out string err)
        {
            try
            {
                switch (CommcationType)
                {
                    case CommcationType_Enum.SerialPort:
                        sp.Write(b, 0, b.Length);
                        break;
                    case CommcationType_Enum.TCP:
                        socket.Send(b);
                        break;
                }
                err = "";
            ...
            catch (Exception exc) { err = exc.Message; ; return false; }
        }

        /// <summary>尝试从通讯对象中接收数据
        /// 
        /// </summary>
        /// <returns>未收到数据则返回空</returns>
        byte[] CommcationReceData()
        {
            byte[] b = null;
            switch (CommcationType)
            {
        ...
        {
            var cmd_temp = new AtlsCommand(CommcationType, 150, 0, prostr);
        ...
        public void SetDO(int index, bool enable)
        {
            ...
            AtlsCommand a = new AtlsCommand(CommcationType, 200, 1, message);
        ...
        {
            AtlsCommand a = new AtlsCommand(CommcationType, 50, 1, vin);
            lock (SendDataCache)
            {
                SendDataCache.Add(a);
            }
        ...
        /// <summary>将Byte数组转换为一个能被人识别的16进制数的字符串形式
        /// 
        /// </summary>
        /// <param name="b"></param>
        /// <returns></returns>
        static string ToStandardString(byte[] b, bool WithNo0X, int len)
        {
            string s = "";
            for (int i = 0; i < len; i++)
            {
                s += (WithNo0X ? "0x" : "") + Convert.ToString(b[i], 16).PadLeft(2, '0') + " ";
            }
            return s;
        }
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        static string ToStandardString(byte[] b, bool WithNo0X)
        {
            return ToStandardString(b, WithNo0X, b.Length);
        }
        ...
        void SocketSendData()
        {
            while (true)
            {
                ...
                #region 通讯重新连接
                bool iscommcationclose = CommcationType == CommcationType_Enum.SerialPort ? !sp.IsOpen : (socket == null || !socket.Connected);
                if (iscommcationclose)//Socket重连接
                {
                    bool recommcationok = false;
                    switch (CommcationType)
                    {
                        case CommcationType_Enum.SerialPort:
                            try
                ...
                if ((DateTime.Now - LastReceHeartTime).TotalSeconds >= 15 || !IsCmd1Connect)
                {
                    ...
                    if (CommcationSendData(Cmd_Connect.命令_ByteList,out string err))
                    {
                        TraceWrite("驱动发送数据成功,命令号:" + Cmd_Connect.命令号);
                    ...
                    AppMessage.Add("驱动发送数据,命令号:" + Cmd_Connect.命令号 + ",长度 " + Cmd_Connect.命令_ByteList.Length, AppMessage.MsgType.阿特拉斯串口指令, false, AppMessage.ImportantEnum.Normal, ToStandardString(Cmd_Connect.命令_ByteList,false));
                }
                ...
                try
                {
                    CommcationSendData(cmd.命令_ByteList, out _);
                    if (cmd.命令号 != 9999)
                    {
                        TraceWrite("驱动发送数据成功,命令号:" + cmd.命令号 + ",长度 " + cmd.命令_ByteList.Length);
                        //AppMessage.Add("Socket发送数据成功,命令号:" + cmd.命令号 + ",长度 " + cmd.命令_ByteList.Length, AppMessage.MsgType.阿特拉斯网络指令);
                        AppMessage.Add("驱动发送数据成功,命令号:" + cmd.命令号 + ",长度 " + cmd.命令_ByteList.Length, AppMessage.MsgType.阿特拉斯串口指令, false, AppMessage.ImportantEnum.Normal, ToStandardString(cmd.命令_ByteList,false));
                    }
                }
                catch /*(Exception e)*/
                {
                    TraceWrite("驱动发送数据失败,命令号:" + cmd.命令号 + ",长度 " + cmd.命令_ByteList.Length);
                    //AppMessage.Add("Socket发送数据失败,命令号:" + cmd.命令号 + ",长度 " + cmd.命令_ByteList.Length, AppMessage.MsgType.阿特拉斯网络指令);
                    AppMessage.Add("驱动发送数据失败,命令号:" + cmd.命令号 + ",长度 " + cmd.命令_ByteList.Length, AppMessage.MsgType.阿特拉斯串口指令, false, AppMessage.ImportantEnum.Normal, ToStandardString(cmd.命令_ByteList,false));
                }
        ...
        void SocketReceiveData()
        {
            while (true)
            {
                ...
                var b = CommcationReceData();
                if (b!=null)
                {
                    AppMessage.Add("接收串口数据,长度:" + b.Length, AppMessage.MsgType.阿特拉斯串口指令, false, AppMessage.ImportantEnum.Normal, ToStandardString(b,false));
        ...
        public class AtlsCommand
        {
            ...
            public AtlsCommand(CommcationType_Enum as_CommcationType, int as_命令号, int as_命令版本=0, string as_命令消息体="")
            {
                命令号 = as_命令号;
                命令版本 = as_命令版本;
                命令消息体 = as_命令消息体;

                switch (as_CommcationType)
                {
                    case CommcationType_Enum.SerialPort:
        ...
        public override bool DC_IsConnected
        {
            get
            {
                return CommcationType == CommcationType_Enum.SerialPort ?
                  (sp == null ? false : sp.IsOpen) :
                  (socket == null ? false : socket.Connected);
            }
        }
